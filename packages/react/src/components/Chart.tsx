        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={item.dataKey}
              className={cn(
                '[&>svg]:text-typography-50 flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5',
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  >
                    <div className="grid gap-1.5">
                      {nestLabel ? tooltipLabel : null}
                      <span className="text-typography-50">
                        {itemConfig?.label || item.name}
                      </span>
                    </div>
                    {item.value && (
                      <span className="text-typography-50 font-mono font-medium tabular-nums">
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              '[&>svg]:text-typography-50 flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3',
            )}
          >
